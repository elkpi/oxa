...
use oxa_chatcompletions::{
    Chunk, Config, Request, Response, StreamDecoder, StreamEncoder, decode_request,
...
impl oxa_vectest::StreamConverter for StreamVectorConverter {
    ...
    fn apply_ir_event(
        ...
        let (chunks, losses) = self.encoder.apply(event).map_err(|err| err.to_string())?;
        let values = chunks
            .into_iter()
            .map(|c| serde_json::to_value(c).map_err(|err| err.to_string()))
